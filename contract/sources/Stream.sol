...
contract CampaignManager {
    ...
    function createCampaign(address tokenAddress, uint256 poolAmount) public {
        ...
        require(
            token.transferFrom(msg.sender, address(this), poolAmount),
    ...
    function distributeRewardsBatch(
        ...
        require(
            userAddresses.length == likes.length,
            "Mismatched user and like arrays"
        );
        ...
        for (uint256 j = 0; j < userAddresses.length; j++) {
            ...
            require(
                token.transfer(userAddresses[j], reward),
    ...
    function getCampaign(
        uint256 campaignId
    ) public view validCampaign(campaignId) returns (address, uint256) {
